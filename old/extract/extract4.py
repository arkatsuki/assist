...
def write_output_file(path_info):
    pass

def put_dict (filePathListFun, outputFun, currentBaseInfoFun):
    """
    ...
    for pathItem in file_path_set_fun:
        ...
        outputFun.append(currentBaseInfoFun[0])    # 版本号
        outputFun.append('\t')
        outputFun.append(currentBaseInfoFun[1])   # 提交人
        outputFun.append('\t')

        action = pathItem[0]

        path_item_path = pathItem[1:]
        idxDev = path_item_path.find(branch_prefix)
...
while j < logLen- 1:  # 最后一行--------没有价值，反而会让下面的if越界
    ...
    if svninfo[j].startswith('Changed paths'):
        ...
        while svninfo[j].strip() != '':
            ...
            # filePathList.append(svninfo[j].strip())
            j = j + 1
        put_dict(file_path_list, output, current_base_info)
        continue
    j = j + 1

if filter_category_bool:
    pass
else:
    pass
